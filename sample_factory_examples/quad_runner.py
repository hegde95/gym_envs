...
python -m sample_factory_examples.quad_runner --algo=APPO --env=my_custom_env_v1 --experiment=halfcheetah_soft --save_every_sec=5 --experiment_summaries_interval=10 --num_workers=24 --num_envs_per_worker=20

After training for a desired period of time, evaluate the policy by running:
python -m sample_factory_examples.enjoy_quad --algo=APPO --env=my_custom_env_v1 --experiment=halfcheetah_soft
...
def make_quad_function():
    # return gym.make("laikago-v2", motor_control_mode = "SYMMETRIC_TORQUE")
    return gym.make("half-cheetah-soft-v0")


def make_custom_env_func(full_env_name, cfg=None, env_config=None):
    # return CustomEnv(full_env_name, cfg)
    return make_quad_function()
...
class CustomEncoder(EncoderBase):
    def __init__(self, cfg, obs_space, timing):
        ...
        # ]
        # print(obs_shape['obs'][0])
        linear_layers = [
            nn.Linear(obs_shape['obs'][0], 400), nonlinearity(cfg),
            nn.Linear(400, 400), nonlinearity(cfg),
            nn.Linear(400, 200), nonlinearity(cfg),
            ]

        self.lin_head = nn.Sequential(*linear_layers)
        self.lin_head_out_size = calc_num_elements(self.lin_head, obs_shape.obs)

        self.init_fc_blocks(self.lin_head_out_size)

    def forward(self, obs_dict):
        # we always work with dictionary observations. Primary observation is available with the key 'obs'
        main_obs = obs_dict['obs']

        x = self.lin_head(main_obs)
        x = x.view(-1, self.lin_head_out_size)
